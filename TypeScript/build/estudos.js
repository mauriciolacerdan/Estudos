...
// Tipando variável:
// Type string:
let tecnologia; // A variável sempre vai ser string
...
let valor; // A variável sempre vai ser number
...
// Union Type
let userId; // Permite que a variável seja do tipo number ou string.
userId = 123;
// userId = "123";
console.log(userId);
...
// Type array:
let numeros; // Array de number que só permite push de number também
numeros = [1, 32, 2, 43];
numeros.push(55);
console.log("Meus Números: ", numeros);
// Type array union:
let numeroes; // Array que permite string e number
numeroes = [1, "plamundo", 2, "43"];
numeroes.push(55, "ds");
console.log("Meus Números com string: ", numeroes);
// Tupla (não possui no JS, apenas no TS):
let aluno; // Array que recebe primeiro uma string, depois um number e depois uma string, seguindo essa ordem
...
let nome; // Usado apenas em alguns casos, quando pode ser null ou outra coisa
nome = "Maurício";
...
let statusAtual = 1;
let mudaStatus = 0;
mudaStatus = statusAtual; // Está afirmando que statusAtual é de fato um number. Isso é um type assertion.
...
function cadastro(email, senha, nome = "Aluno", // Quando coloca um =, ele retorna o que foi passado ou, caso não passe nada, retorna "Aluno"
idade) {
